    ...
    if filename.endswith(".csv"):
        ...
        for i in range(1, len(imu[imu.columns[8]])):
            ...
            if diff > threshold_IC:
                if prev_time_index!=0 and (imu[imu.columns[9]][i]-imu[imu.columns[9]][prev_time_index]) <time_protection :
                    continue
                if (i-1)<len(TOtime) and i>=1 and (abs(TOtime[i]- imu[imu.columns[9]][i])<200 or abs(TOtime[i-1]- imu[imu.columns[9]][i])<200 or abs(TOtime[i+1]- imu[imu.columns[9]][i])<200 ):
                    continue
